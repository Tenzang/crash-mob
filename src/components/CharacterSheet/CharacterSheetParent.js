...
class CharacterSheetParent extends Component {
  constructor() {
    ...
    this.state = {
      characterId: "",
      abilities: {
        // will update with data from SERVER
      ...
      proficiencyMod: 2,
      URL: {
        character: sourceURL + "/characters/",
        score: sourceURL + "scores/",
        skills: sourceURL + "skills/",
        abilities: sourceURL + "abilities/",
      },
      name: "???",
      race: "???",
      role: "???",
  ...
  componentDidMount() {
    ...
    const fetchData = (URL) => {
      ...
      axios(URL.score + characterID + ".json").then((response) => {
        ...
        for (const key in data) {
          newAbilities.abilities[key] = {
            score: data[key],
            modifier: modifier(data[key]),
          };
        }
  ...
  render() {
    ...
    const sheetStyle = {
      ...
      width: "50%",
      margin: "5% auto",
      border: "2px solid rgba(46, 71, 96, 1)",
    };
    const paperStyle = {
      display: "grid",
      gridTemplateColumns: "0.5fr 0.8fr 0.8fr 0.8fr",
      gridGap: "1%",
      paddingBottom: "2.5%",
      paddingRight: "2.5%",
      paddingLeft: "2.5%",
    };
    const tempDiceThrows = {
      display: "grid",

...
class SkillProficiencies extends Component {
  ...
  componentDidMount() {
    ...
    axios
      .get("https://www.dnd5eapi.co/api/races/" + this.state.race.toLowerCase())
      .then((res) => {
        const race_proficiencies = res.data.starting_proficiencies.map(
          ({ name }) => name
        );
        this.setState({ race_proficiencies });
        this.props.knownSkills(race_proficiencies);
        const race_proficiencies_options =
          res.data.starting_proficiency_options.from;
        this.setState({ race_proficiencies_options });
      });

    axios
      .get(
        "https://www.dnd5eapi.co/api/classes/" + this.state.role.toLowerCase()
      )
      .then((res) => {
        ...
        const extra_class_proficiency_options =
          res.data.proficiency_choices[1].from;
      ...
      <MuiThemeProvider>
        <>
          <Dialog open fullWidth maxWidth="sm">
            ...
            <p>
              {this.state.race_proficiencies.map((name) => {
            ...
            {this.state.extra_class_proficiency_options.length > 0 ? (
              <h3>Extra Class Proficiencies Choices</h3>
            ) : (
              <p></p>
            )}
            {this.state.extra_class_proficiency_options.length > 0 ? (
